...
class UKHitFactory:
    def __init__(self, seed):
        ...
        self.params = self._derive_song_parameters()

    def _derive_song_parameters(self):
        params = {}
        params['bpm'] = random.randint(100, 140)
        for track_num in range(self.midi_obj.numTracks):
            self.midi_obj.addTempo(track_num, 0, params['bpm'])

        params['target_duration_seconds'] = random.randint(180, 220)

        keys_info = [
            (0, "C Major", True), (2, "D Major", True), (4, "E Major", False),
            (5, "F Major", True), (7, "G Major", True), (9, "A Major", False),
            (9, "A Minor", False), (4, "E Minor", False), (2, "D Minor", False)
        ]
        common_keys = [ (0, "C Major", True), (7, "G Major", True), (5, "F Major", True),
                        (9, "A Minor", False), (4, "E Minor", False)]
        if random.random() < 0.8:
            chosen_key_root, chosen_key_name, chosen_is_major = random.choice(common_keys)
        else:
            chosen_key_root, chosen_key_name, chosen_is_major = random.choice(keys_info)
        params['key_root_original'] = chosen_key_root
        params['is_major_original'] = chosen_is_major
        ...
        form_templates = [
            ...
            ["Intro", "Verse", "Chorus", "Verse", "Chorus", "InstrumentalHook", "Chorus", "Outro"],
            ["Intro", "Verse", "PreChorus", "Chorus", "InstrumentalHook", "PreChorus", "Chorus", "Chorus", "Outro"]
        ]
        params['song_form'] = random.choice(form_templates)

        schemas = {
            "I-V-vi-IV": ([0, 7, 9, 5], "Classic Pop/Rock"),
            "vi-IV-I-V": ([9, 5, 0, 7], "Singer/Songwriter/Ballad"),
            "I-vi-IV-V": ([0, 9, 5, 7], "Doo-wop/Oldies"),
            "IV-V-vi-I": ([5, 7, 9, 0], "Modern Pop/Hopscotch")
        }
        if random.random() < 0.6:
            params['harmonic_schema_name'] = "I-V-vi-IV"
        else:
            params['harmonic_schema_name'] = random.choice(list(schemas.keys()))
        ...
        params['use_7th_chords'] = random.random() < 0.7

        params['rhythm_personality'] = random.choice(["PopRock", "HipHopGroove", "EDMPulse"])
        print(f"Rhythm Personality: {params['rhythm_personality']}")

        params['instruments'] = {
            "Drums": None, "Bass": random.choice([33, 34, 38]),
            "Chords": random.choice([0, 4, 88]), "Melody": random.choice([80, 25, 52]),
            "Pad": random.choice([89, 90, 92])
        }
        if params['rhythm_personality'] in ["HipHopGroove", "EDMPulse"]:
            params['instruments']['Bass'] = random.choice([38, 39])

        params['overall_dynamic_level'] = random.choice([70, 80, 90])
        params['melodic_complexity_level'] = random.choice(["Simple", "Moderate"])
    ...
    def _get_chord_notes_from_roman(self, roman_numeral_degree, key_root, is_major, use_7th=False):
    ...
    def _get_chord_progression_for_section(self, section_type, num_bars):
        ...
        use_7th_chords_for_section = self.params['use_7th_chords'] and random.random() < 0.7
        ...
        if section_type == "Bridge" and self.params.get('bridge_is_modulating', False) and random.random() < 0.8:
            ...
            for bar in range(num_bars):
                ...
                root_note, chord_type = self._get_chord_notes_from_roman(degree, key_root, is_major, use_7th_chords_for_section)
        ...
        for i, (root_midi, chord_type, duration_beats) in enumerate(chord_prog):
            ...
            if is_peak and section_type == "Chorus":
                ...
                for step_idx in range(num_steps):
                    ...
                    if beat_in_chord + actual_step_duration > current_beat + duration_beats:
                        actual_step_duration = (current_beat + duration_beats) - beat_in_chord
                    if actual_step_duration <= 0.05: continue
                    note_to_play = bass_octave_root
                    note_velocity = current_note_overall_velocity
                    ...
                    if rhythm_personality == "HipHopGroove" and random.random() < 0.4 and step_idx == 0:
                        ...
                        self.midi_obj.addNote(track_num, track_num, note_to_play, beat_in_chord, duration_beats - 0.1, note_velocity)
                        break
    ...
    def _generate_melodic_phrase(self, num_beats, current_chord_root, current_chord_type, key_root, is_major, complexity, section_profile, is_hook_on_downbeat_section, motif_to_develop=None, is_motif_repetition=False):
        ...
        scale_choice = "major_pentatonic" if is_major else "minor_pentatonic"
        if melody_style == 'bridge_distinct' or random.random() < 0.3:
            scale_choice = "diatonic"
        ...
        if motif_to_develop and is_motif_repetition and random.random() < 0.6:
            ...
            if variation_choice != "none":
                current_motif_instance = self._apply_motif_variation(motif_to_develop, variation_choice)
        ...
        while current_mel_beat < num_beats - 0.125 and num_notes_in_phrase < (num_beats * 2.0 * rhythmic_density_for_walk):
            ...
            if last_pitch is not None:
                interval = abs(next_pitch - last_pitch); attempts = 0; max_interval = 5 if complexity == "Simple" else (9 if melody_style == 'bridge_distinct' else 7)
    ...
    def _add_melody_line(self, track_num, chord_prog, start_time_beats, section_type, section_profile):
        ...
        if is_hook_section and not self.params.get(hook_motif_key) and section_profile.get('melody_style') != 'bridge_distinct':
            ...
            self.params[hook_motif_key] = self._create_melodic_motif(
                random.choice([1.0, 2.0]), motif_chord_root, motif_chord_type, 
                key_root, is_major, complexity, section_profile.get('melody_style', 'standard')
            )
        ...
        for i, (chord_root_midi, chord_type, chord_duration_beats) in enumerate(chord_prog):
            ...
            melodic_phrase_notes_data = self._generate_melodic_phrase(
                chord_duration_beats, chord_root_midi, chord_type, key_root, is_major,
                complexity, section_profile, is_hook_section,
            ...
            if is_hook_section and i == 0 and section_profile.get('melody_style') != 'bridge_distinct':
                 self.params[f"{hook_motif_key}_used_once"] = True
    ...
    def _generate_section_midi(self, section_type, current_time_beats, section_bars):
        ...
        if section_type == "Bridge" and section_profile.get('modulate_key', False):
            self.params['bridge_is_modulating'] = True
            modulation_target = random.choice([7, 5])
    ...
    def compose(self):
        ...
        for track_name, track_num in self.track_map.items():
            ...
            if track_name != "Drums" and track_name in self.params['instruments'] and self.params['instruments'][track_name] is not None:
        ...
        for section_name_in_form in set(self.params['song_form']):
            if section_name_in_form in ["Intro", "Outro"]: section_bar_lengths[section_name_in_form] = random.choice([4, 8])
...
if __name__ == "__main__":
    song_seed = random.randint(0, 1000000)
    # song_seed = 123456 # For testing Phase 6
    print(f"Using seed: {song_seed} for local test.")
    hit_generator = UKHitFactory(seed=song_seed)
